...
class MobileAdmin(admin.ModelAdmin):
    list_display = (
        'id', 'get_user_name', 'get_mobile_imei', 'get_mobile_type', 'returned', 'number_of_installments_payed',
        'next_installment_due', 'installment_amount')
    raw_id_fields = ('sold_item', 'customer', 'insurer_one', 'insurer_two')
    list_filter = ('is_complete', 'returned', ('next_installment_due', DateRangeFilter))
    search_fields = (
        'customer__user__name', 'sold_item__imei_number', 'sold_item__type', 'number_of_installments_payed')
    ...
    def get_mobile_imei(self, obj):
        return obj.sold_item.imei_number

    get_mobile_imei.short_description = "imei_number"

    def get_mobile_type(self, obj):
        return obj.sold_item.type

    get_mobile_imei.short_description = "mobile_type"
